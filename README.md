...
큰 흐름은 아래와 같습니다 (세부 조건·수치는 이 아래 텍스트 다이어그램 참조).

```mermaid
flowchart TD
    A[영상 파일] --> B

    subgraph 신호["신호 추출 · 검증"]
        B["1~5. 원본 신호 추출 (병렬)<br/>Whisper 전사 · 장면 탐지 · 음성 음량 ·<br/>전체 음량 · 프레임 밝기"]
        C["6~7. 장면·텍스트 검증 (병렬)<br/>CLIP 배치 장면 검증 · 텍스트 유사도"]
        B --> C
    end

    subgraph 생성["마커 후보 생성 (관문)"]
        D["8. Path 1 침묵 · Path 2 컷 앵커 · Path 3 페이드<br/>→ 조건을 만족하는 마커 확정"]
    end

    subgraph 우선순위["점수화 · 우선순위"]
        E["9. CLIP 개별 재검증"]
        F["10. 점수화 → 1차/2차 XML 생성"]
        E --> F
    end
