...
2. **마커는 장면 전환 기반으로 잡는다.** 침묵만으로 마커를 잡으면 같은 공간 안의 발화 중단에도 마커가 생깁니다. CLIP으로 확인된 진짜 장면 전환 + 문장 경계가 겹치는 지점이 가장 강한 후보입니다.
...
영상 파일
  ...
  ├─► [2] PySceneDetect 장면 탐지  → 컷 시각 목록
  └─► [3] ffmpeg 음성 음량 곡선    → 침묵 판별용 dB 엔벌로프
       (1~3 병렬 실행)
  │
  ▼
  [4] CLIP 배치 장면 검증  (ViT-B-32)
      PySceneDetect가 찾은 컷마다 전후 프레임을 비교
      코사인 유사도 < 0.60 → "진짜 장면 전환" 확정
      유사도 ≥ 0.60 → 앵글 변화·그래픽·점프컷으로 판단, 제외
  │
  ▼
  [5] 한국어 텍스트 유사도  (jhgan/ko-sroberta-multitask)
      데드존 밖 전체 컷 지점의 전후 30초 발화를 임베딩해 유사도 계산
      (CLIP 확정 여부와 무관하게 모든 컷에 계산)
      유사도 < 0.75 → 주제가 바뀌는 컷으로 판단 (+2.0점)
  │
  ▼
  [6] 마커 후보 생성  (두 가지 경로)
      ...
      Path 2 — 컷 앵커
        CLIP 확인 컷이 두 문장 경계 사이(±0.5s)에 정확히 떨어지는 경우
        → 침묵 없이 장면 전환 시각에서 가장 가까운 허용 프레임에 마커 생성
  │
  ▼
  [7] CLIP 개별 재검증
      대상: Path 1 중 일반 컷(±0.3s) 기반 마커 + Path 2 컷 앵커 마커
      (배치 CLIP으로 이미 확인된 ±1.0s CLIP 업그레이드 마커는 생략)
      단건으로 재확인 후 유사도 ≥ 0.78이면 "참고"로 강등 (마커 자체는 유지)
  │
  ▼
  [8] 점수화 + 1차/2차 분리
      1차: 간격 규칙(첫 광고 3~10분, 이후 10~15분 간격) 적용 → 슬롯당 1+4 후보
      2차: 제한 없이 조건 만족 마커 전체
...
처음 실행 시 CLIP 모델(ViT-B-32, ~350MB)과 ko-sroberta 모델(~400MB)이 자동 다운로드됩니다.  
...
- `{영상명}_{크기}.transcript2.json` — Whisper 전사 결과
- `{영상명}_{크기}.scenes.json` — 장면 전환 목록
- `{영상명}_{크기}.voice.json` — 음성 음량 엔벌로프
- `{영상명}_{크기}.clip_cuts.json` — CLIP 유사도 결과
- `{영상명}_{크기}.text_sim.json` — 텍스트 유사도 결과
...
현재 성능 기준치 (40편, 4,220 마커, 양성 비율 1.3%):

- Logistic Regression: PR-AUC 0.057, ROC-AUC 0.687
- Random Forest: PR-AUC 0.021, ROC-AUC 0.608

PR-AUC가 낮은 것은 예상된 결과입니다. YBJ는 고정 스튜디오 포맷이라 광고 구간과 일반 구간의 시각적 특징이 거의 동일하고, 광고 지점 자체가 편집자 개인 판단이 작용하는 주관적 라벨입니다. 이 시스템의 목표는 ML 분류기가 아니라 조건 기반 후보 제시입니다.
...
| 장면 전환 탐지 | PySceneDetect (ContentDetector) |
| 장면 전환 검증 | open-clip ViT-B-32-quickgelu (SAME_THRESHOLD=0.60) |
